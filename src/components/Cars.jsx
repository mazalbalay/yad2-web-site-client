...
export default function Cars() {
  ...
  const getData = async () => {
    if (location == "/login") {
      const { data } = await axios.get(`${api}/cars/${selctor.state._id}`);
      setData(data);
    } else {
  ...
  const likeFun = async (id) => {
    if (selctor?.state) {
      ...
      await axios.put(`${api}/cars/${id}`, post);
      if (location == `/cars`) {
        setLike(!like);
      }
    } else {
      ...
      {data ? (
        <div className="flex flex-col justify-start items-center pb-10">
          {location === "/login" ? (
            ...
            <h1 className="text-4xl mt-32 font-mono">cars</h1>
          )}
          {location === `/cars` ? (
