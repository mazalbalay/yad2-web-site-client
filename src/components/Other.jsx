...
export default function Other() {
  ...
  const getData = async () => {
    if (location == "/login") {
      const { data } = await axios.get(
        `http://localhost:8000/other/${selctor.state._id}`,
      );
      setData(data);
    } else {
      const { data } = await axios.get(`http://localhost:8000/other`);
      setData(data);
    }
  };

  const deleteData = async (id) => {
    await axios.delete(`http://localhost:8000/other/${id}`);
    console.log("deleted");
  };
  ...
  const likeFun = async (id) => {
    if (selctor?.state) {
      ...
      await axios.put(`http://localhost:8000/other/${id}`, post);
      if (location == `/other`) {
        setLike(!like);
      }
    } else {
  ...
  return (
    <div className="flex flex-col justify-start items-center pb-10">
      {location === "/login" ? (
        ...
        <h1 className="text-4xl mt-32 font-mono">other</h1>
      )}
      {location === `/other` ? (
